...
const { VITE_APP_BASE_URL, VITE_APP_BASE_URL_PREFIX } = import.meta.env;
...
const transform: AxiosTransform = {
  ...
  requestInterceptors: (config: AxiosRequestConfig, options: CreateAxiosOptions) => {
    // 请求之前处理config TODO:修改token name
    const token = localStorage.getItem('CPVF_TOKEN__');
  ...
  responseInterceptorsCatch: async (error: any) => {
    ...
    const { isTransformResponse, cancelToken } = config;
    if (cancelToken) {
